...
startGame = function() {
    Alien_Game.setBoard(0,new Starfield(20,0.4,100,true));
    Alien_Game.setBoard(1,new Starfield(50,0.6,100));
    Alien_Game.setBoard(2,new Starfield(100,1.0,50));
    Alien_Game.setBoard(3,new TitleScreen("Alien Invasion", 
                                    "Press fire to start playing",
                                    Alien_playGame));
	Alien_Game.setBoard(5,new Alien_GamePoints(0));
};
...
sigLevel = function(){
    Alien_Game.setBoard(3,new TitleScreen("Next level!", 
...
Starfield = function(speed,opacity,numStars,clear) {
    ...
    var stars = $('<canvas/>')
	.attr('width', Alien_Game.width)
	.attr('height', Alien_Game.height)[0];
...
PlayerShip = function() { 
    ...
    this.x = Alien_Game.width/2 - this.w / 2;
    this.y = Alien_Game.height - 10 - this.h;

    this.step = function(dt) {
	if(Alien_Game.keys['left']) { this.vx = -this.maxVel; }
	else if(Alien_Game.keys['right']) { this.vx = this.maxVel; }
	else { this.vx = 0; }

	this.x += this.vx * dt;

	if(this.x < 0) { this.x = 0; }
	else if(this.x > Alien_Game.width - this.w) { 
	    this.x = Alien_Game.width - this.w;
	}

	this.reload-=dt;
	this.reload2-=dt;
		
	if(!Alien_Game.keys['fire']) this.up = true;
	if(this.up && Alien_Game.keys['fire'] && this.reload < 0) {
	    this.up=false;
	...
	if (this.up && this.reload2<0 && (Alien_Game.keys['fireball_l'] || Alien_Game.keys['fireball_r']) ){
			this.dir_l = Alien_Game.keys['fireball_l'];		//if true goes to left, otherwise goes to right
			this.up=false;
			this.reload2=this.reloadTime2;
...
FireBall.prototype.step = function(dt)  {
    ...
    if(collision) {
      this.board.add(new Explosion(this.x + this.w/2, 
	    ...
	    collision.hit(this.damage);
      Alien_Game.points += this.points || 100;
	    this.board.remove(collision);
      } else if(this.y < -this.h) { 
	    this.board.remove(collision); 
    }
...
Enemy.prototype.step = function(dt) {
   ...
   if(this.reload <= 0 && Math.random() < (this.firePercentage || 0.01) ) {
    ...
    if(this.y > Alien_Game.height ||
       this.x < -this.w ||
       this.x > Alien_Game.width) {
	this.board.remove(this);
    }
};
...
EnemyMissile.prototype.step = function(dt)  {
	...
	collision.hit(this.damage);
	this.board.remove(this);
    } else if(this.y > Alien_Game.height) {
	this.board.remove(this); 
    }
};
...
$(function() {
    Alien_Game.initialize("game",sprites,startGame);
});
