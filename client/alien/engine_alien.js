...
Alien_Game = new function() {                                                                  
	...
	// encima de los botones t�ctiles
	this.playerOffset = 10;

	// En pantallas t�ctiles grandes haremos que el tama�o de la
	// ventana sea el doble del tama�o del canvas en pixels, para
	// mejorar as� el rendimiento al tener que procesar menos
	// pixels.  En estas pantallas grandes esta propiedad ser�
	// 2. Se determina en this.setupMobile() Cuando se hagan
	// c�lculos de eventos de toque hay que tener en cuenta este
	...
	Alien_SpriteSheet.load(sprite_data,callback);
	...
	$(window).keydown(function(event){
	    if (KEY_CODES[event.which]) {
			if(document.activeElement.id == ""){
				Alien_Game.keys[KEY_CODES[event.which]] = true;
				return false;
			}else{
				return true;
			}
	...
	$(window).keyup(function(event){
	    if (KEY_CODES[event.which]) {
		Alien_Game.keys[KEY_CODES[event.which]] = false;
		return false;
	    }
	});
	...
	for(var i=0,len = boards.length;i<len;i++) {
	    if(boards[i]) { 
		boards[i].step(dt);
		boards[i].draw(Alien_Game.ctx);
	    }
	}

	// Ejecutar dentro de 30 ms
	setTimeout(Alien_Game.loop,30);
    };
	...
	// Comprobar si el usuario est� en modo landscape
	// Si no lo est�, pedirle que rote el dispositivo 
	if(w > h) {
	    alert("Please rotate the device and then click OK");
	    w = window.innerWidth; h = window.innerHeight;
	}

	// Cambiar el tama�o del div container para que sea mayor que
	// la p�gina para permitir eliminar la barra de direcci�n del
	// navegador
	container.style.height = h*2 + "px";

	// Desplazar la ventana m�nimamente para forzar a que desaparezca
	// la barra de direcci�n del navegador
	window.scrollTo(0,1);

	// Cambiar el tama�o del contenedor para que sea el de la ventana
	h = window.innerHeight + 2;
	container.style.height = h + "px";
	container.style.width = w + "px";
	container.style.padding = 0;

	//  Comprobar si estamos en un dispositivo t�ctil grande (tableta)
	if(h >= this.canvas.height * 1.75 || w >= this.canvas.height * 1.75) {
	    // Si grande, tama�o del canvas = mitad del tama�o de la ventana
            // As� se mejora el rendimiento al tener un canvas m�s peque�o
	    ...
	    this.canvas.style.height = h + "px";
            ...
            //  Si no grande, tama�o del canvas = tama�o de la ventana
	    this.canvas.width = w;
	    this.canvas.height = h;
	}

	//  Poner el canvas en una posici�n absoluta en la esquina
...
Alien_SpriteSheet = new function() {
...
TitleScreen = function TitleScreen(title,subtitle,callback) {
	...
	if(!Alien_Game.keys['fire']) up = true;
	if(up && Alien_Game.keys['fire'] && callback) callback();
    };
	...
	ctx.fillText(title,Alien_Game.width/2,Alien_Game.height/2);

	ctx.font = "bold 20px bangers";
	ctx.fillText(subtitle,Alien_Game.width/2,Alien_Game.height/2 + 40);
    };
};
...
Sprite.prototype.setup = function(sprite,props) {
    ...
    this.w =  Alien_SpriteSheet.map[sprite].w;
    this.h =  Alien_SpriteSheet.map[sprite].h;
}
...
Sprite.prototype.draw = function(ctx) {
    Alien_SpriteSheet.draw(ctx,this.sprite,this.x,this.y,this.frame);
}
...
TouchControls = function() {
    ...
    var unitWidth = Alien_Game.width/5;
	...
	var yLoc = Alien_Game.height - unitWidth;
	this.drawSquare(ctx,gutterWidth,yLoc,"\u25C0", Alien_Game.keys['left']);
	this.drawSquare(ctx,unitWidth + gutterWidth,yLoc,"\u25B6", Alien_Game.keys['right']);
	this.drawSquare(ctx,4*unitWidth,yLoc,"| |",Alien_Game.keys['fire']);
	this.drawSquare(ctx,3*unitWidth,yLoc,"\u25C9",Alien_Game.keys['fireball_l']||Alien_Game.keys['fireball_r'])
    ...
    this.fireballFinger_id=0;
    this.fireballFinger_x=0;
    this.fireballFinger_y=0;
	...
	Alien_Game.keys['left'] = false;
	Alien_Game.keys['right'] = false;
	Alien_Game.keys['fireball_l'] = false;
	Alien_Game.keys['fireball_r'] = false;
	for(var i=0;i<e.targetTouches.length;i++) {
	    ...
	    x = touch.pageX / Alien_Game.canvasMultiplier - Alien_Game.canvas.offsetLeft;
	    if(x < unitWidth) {
	    	Alien_Game.keys['left'] = true;
	    } 
	    if(x > unitWidth && x < 2*unitWidth) {
	    	Alien_Game.keys['right'] = true;
	    }
		if(x>3*unitWidth && x<4*unitWidth){
			if (e.type=='touchstart'){
				this.fireballFinger_id=touch.identifier;
				this.fireballFinger_x=touch.pageX;
				this.fireballFinger_y=touch.pageY;
			}
			if(touch.identifier == this.fireballFinger_id && e.type=='touchmove'){
				if (touch.pageY < this.fireballFinger_y){
					touch.pageX < this.fireballFinger_x ? Alien_Game.keys['fireball_l']=true : Alien_Game.keys['fireball_r']=true;
				}
			}
		}
	}
	...
	if(e.type == 'touchstart' || e.type == 'touchend') {
		...
		x = touch.pageX / Alien_Game.canvasMultiplier - Alien_Game.canvas.offsetLeft;
		if(x > 4 * unitWidth) {
		    Alien_Game.keys['fire'] = (e.type == 'touchstart');
		}
	    }
	}
    };
    ...
    Alien_Game.canvas.addEventListener('touchstart',this.trackTouch,true);
    Alien_Game.canvas.addEventListener('touchmove',this.trackTouch,true);
    Alien_Game.canvas.addEventListener('touchend',this.trackTouch,true);
    ...
    // offset.
    Alien_Game.playerOffset = unitWidth + 20;
};

Alien_GamePoints = function() {
  Alien_Game.points = 0;
