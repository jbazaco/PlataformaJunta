...
TouchControls = function() {
	...
	this.drawSquare(ctx,3*unitWidth,yLoc,"\u25C9",Game.keys['fireball_l']||Game.keys['fireball_r'])
	...
	Game.keys['fireball_l'] = false;
	Game.keys['fireball_r'] = false;
		...
		if(x>3*unitWidth && x<4*unitWidth){
			...
			if(touch.identifier == this.fireballFinger_id && e.type=='touchmove'){
				if (touch.pageY < this.fireballFinger_y){
					touch.pageX < this.fireballFinger_x ? Game.keys['fireball_l']=true : Game.keys['fireball_r']=true;
				}
			}
		}
	}
